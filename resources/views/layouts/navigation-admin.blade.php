<nav x-data="{ open: false }" class="bg-blue-800 text-white border-b border-blue-900">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('admin.dashboard')" :active="request()->routeIs('admin.dashboard')" class="text-white hover:text-blue-200">
                        {{ __('Dashboard') }}
                    </x-nav-link>

                    <x-nav-link :href="route('admin.users.index')" :active="request()->routeIs('admin.users.*')" class="text-white hover:text-blue-200">
                        {{ __('Usuários') }}
                    </x-nav-link>
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        @if(Auth::user()->hasRole('sysadmin'))
                            <x-dropdown-link :href="route('sysadmin.dashboard')">
                                {{ __('Área SysAdmin') }}
                            </x-dropdown-link>
                        @endif
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <x-dropdown-link :href="route('logout')"
                                    ...
                                    onkeydown="event.key === 'Enter' && (event.preventDefault(); this.closest('form').submit());">
                                {{ __('Log Out') }}
                            </x-dropdown-link>
                        </form>
                    </x-slot>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('admin.dashboard')" :active="request()->routeIs('admin.dashboard')" class="text-white hover:text-blue-200">
                {{ __('Dashboard') }}
            </x-responsive-nav-link>

            <x-responsive-nav-link :href="route('admin.users.index')" :active="request()->routeIs('admin.users.*')" class="text-white hover:text-blue-200">
                {{ __('Usuários') }}
            </x-responsive-nav-link>
        ...
        <div class="pt-4 pb-1 border-t border-blue-700">
            ...
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link :href="route('dashboard')" class="text-white hover:text-blue-200">
                    {{ __('Área do Usuário') }}
                </x-responsive-nav-link>

                @if(Auth::user()->hasRole('sysadmin'))
                    <x-responsive-nav-link :href="route('sysadmin.dashboard')" class="text-white hover:text-blue-200">
                        {{ __('Área SysAdmin') }}
                    </x-responsive-nav-link>
                @endif
