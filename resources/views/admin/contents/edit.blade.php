...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <form action="{{ route('admin.contents.update', $content) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                ...
                <div class="sm:col-span-4">
                    ...
                    <div class="mt-1">
                        <input type="text" name="title" id="title" value="{{ old('title', $content->title) }}" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                    </div>
                </div>
                ...
                <div class="sm:col-span-6">
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $content->slug) }}" required class="flex-1 min-w-0 block w-full px-3 py-2 rounded-none rounded-r-md focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300">
                    </div>
                </div>
                    ...
                    <div class="mt-1">
                        <select id="type" name="type" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                            @foreach($types as $type)
                                <option value="{{ $type->value }}" {{ old('type', $content->type->value) == $type->value ? 'selected' : '' }}>{{ $type->label() }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="mt-1">
                        <select id="visibility" name="visibility" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                            @foreach($visibilities as $visibility)
                                <option value="{{ $visibility->value }}" {{ old('visibility', $content->visibility->value) == $visibility->value ? 'selected' : '' }}>{{ $visibility->label() }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-1">
                        <input type="datetime-local" name="published_at" id="published_at" value="{{ old('published_at', $content->published_at?->format('Y-m-d\TH:i')) }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                    </div>
                </div>
                ...
                <div class="sm:col-span-3 flex items-center h-full pt-6">
                    ...
                    <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $content->is_active) ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
...
<script>
    // Slug Generator apenas no create ou se mexer no titulo deliberadamente
    // Aqui no update, melhor não mudar slug automaticamente para não quebrar links
</script>
@endpush
@endsection
