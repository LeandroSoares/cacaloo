            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="visibility" id="visibility" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="public" {{ old('visibility') == 'public' ? 'selected' : '' }}>Público (Visível na Home)</option>
                        <option value="private" {{ old('visibility') == 'private' ? 'selected' : '' }}>Privado (Apenas Logados)</option>
                    </select>
                    @error('visibility')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" required
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                              placeholder="Breve descrição do evento para aparecer no card...">{{ old('description') }}</textarea>
